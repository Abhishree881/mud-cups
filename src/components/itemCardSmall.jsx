...
function ItemCardSmall(props) {
  let categoryIndex = -1, itemIndex = -1;
  ...
  return (
    <div className="min-w-[130px] h-full flex flex-col gap-[6px] relative">
      ...
      <div className="absolute top-[9px] right-[5px]" onClick={() => {
        const newItem = { ...props.menu[categoryIndex].items[itemIndex], isFavourite: !props.menu[categoryIndex].items[itemIndex].isFavourite }
        props.setItem(newItem, categoryIndex, itemIndex)
        props.setFavourite(categoryIndex, itemIndex)
      }}>
        {props.menu[categoryIndex].items[itemIndex].isFavourite ? <FaHeart color="#eb1727" fontSize={"14px"} /> : <FaRegHeart color="white" fontSize={"14px"} />
        }
      </div>
      <div
