...
import { removeFromCart, updateCart, setActiveItem } from '../Actions/CartActions';


function AddToCart(props) {
    console.log("item", props.activeItem)
    ...
    const HandleClick = (isAdd) => {
        if (isAdd) {
            const newActiveItem = { ...props.activeItem, count: props.activeItem.count + 1 }
        ...
        <div className='w-full h-full flex flex-col justify-between border bg-[#fffaf7] pt-[16px] px-[12px] rounded-tl-[16px] rounded-tr-[16px]' ref={popupRef} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} style={{ transform: `translateX(${offset}px)` }}>
            <div className='h-[70px] w-full flex'>
                <div className='h-full w-full flex-[21] max-w-[70px] bg-gray-200 rounded-[6px]' style={{
                    backgroundImage: `url(${props.activeItem?.imgUrl})`,
                    backgroundSize: "100% 100%",
                }}></div>
                <div className='h-full w-full flex flex-[79] flex-col pl-[12px] overflow-hidden font-[700]'>
                    <span className='text-[18px]'>{props.activeItem?.name}</span>
                    <span className='text-[15px] text-[#00000050]'>{props.activeItem?.description}</span>
                </div>
            </div>

            {props.activeItem?.addOn?.length > 0 && <div className='w-full h-fit font-[700] pb-[6px] pt-[12px]'>Add-Ons</div>}
            {props.activeItem?.addOn?.length > 0 && <div className="w-full h-[0.25px]" style={{ background: "#ded6cd" }}></div>}
            {!props.activeItem?.addOn?.length > 0 && <div className='w-full h-[10px]'></div>}
